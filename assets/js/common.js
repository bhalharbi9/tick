window.APP = {
  ...
  teamAssets: {
    "سبورتنج لشبونة": {
      logo: "https://upload.wikimedia.org/wikipedia/en/c/cb/Logo_Sporting_Clube_de_Portugal.png",
      colors: ["#006e53", "#0fa36b"]
    },
    "بودو جليمت": {
      logo: "https://upload.wikimedia.org/wikipedia/en/thumb/5/55/FK_Bod%C3%B8_Glimt_logo.svg/512px-FK_Bod%C3%B8_Glimt_logo.svg.png",
    ...
    "غلطة سراي": {
      logo: "https://upload.wikimedia.org/wikipedia/commons/5/53/Galatasaray_Sports_Club_Logo.png",
    ...
    "بايرن ميونخ": {
      logo: "https://upload.wikimedia.org/wikipedia/commons/1/1f/FC_Bayern_M%C3%BCnchen_logo_%282017%29.svg",
    ...
    "الهلال": {
      logo: "assets/img/teams/alhilal.png",
      colors: ["#0054a6", "#7ec8ff"]
    },
    "الأهلي": {
      logo: "assets/img/teams/alahli.png",
      colors: ["#007a3d", "#c9a227"]
    }
  }
};
...
function points(ph, pa, rh, ra, pe, re, pp, rp, pw, rw) {
  if ([ph, pa, rh, ra].some(v => v === null || v === undefined || v === "")) {
    return 0;
  }
  ...
  let p = 0;
  let invalidKnockoutPrediction = false;

  // توقع غير منطقي:
  // اختار أشواط أو ترجيح والنتيجة ليست تعادل
  if ((Boolean(pe) || Boolean(pp)) && ph !== pa) {
    invalidKnockoutPrediction = true;
  }

  // لا تمنح نقاط للنتيجة الأساسية إذا كان التوقع غير منطقي
  if (!invalidKnockoutPrediction) {
    if (ph === rh && pa === ra) {
      p = 10;
    } else {
  ...
  // لا تمنح نقاط أشواط إلا إذا:
  // 1) المباراة وصلت فعلًا للأشواط
  // 2) المشارك توقعها
  // 3) التوقع منطقي (أي النتيجة تعادل)
  if (!invalidKnockoutPrediction) {
    if (Boolean(re) && Boolean(pe)) {
      p += 2;
    }
  }

  // تمنح نقاط الترجيح فقط إذا حدث الترجيح فعلًا وتوقعه المشارك
  if (Boolean(rp) && Boolean(pp)) {
    p += 2;
  }

  // الفائز بالترجيح
  if (Boolean(rp) && pw && rw && pw === rw) {
    p += 3;
  }
...
function getAsset(teamName) {
  return window.APP.teamAssets[teamName] || {
    logo: "assets/img/teams/default.png",
...
function getLockTime(fixture) {
  ...
  return kickoff - Number(fixture.lock_minutes_before || 10) * 60000;
}
...
function formatCountdown(ms) {
  ...
  return d > 0
    ? `${d} يوم ${pad(h)}:${pad(m)}:${pad(sec)}`
    : `${pad(h)}:${pad(m)}:${pad(sec)}`;
}

function countdownText(f) {
  return formatCountdown(getLockTime(f) - Date.now());
}
...
function currentLeaderAnnouncement(rows) {
  if (!rows.length) return null;

  const leader = rows[0];
  return {
    title: `المتصدر الحالي: ${leader.full_name}`,
    subtitle: `يمتلك الآن ${leader.total_points} نقطة ويتجه نحو جائزة ${reward(1)}`,
    reward: reward(1)
  };
}

async function fetchParticipantPredictions(participantId) {
