-- فعّل هذا الملف داخل Supabase SQL Editor
...
begin
  ...
  real_outcome := case
    when real_home > real_away then 'H'
    when real_home < real_away then 'A'
    else 'D'
  end;
  ...
  if pred_outcome = real_outcome and pred_diff = real_diff then
    points := 7;
  elsif pred_outcome = real_outcome then
    points := 5;
  end if;

  if pred_home = real_home or pred_away = real_away then
    points := points + 3;
  end if;
  ...
  if distance = 1 then
    points := points + 2;
  elsif distance = 2 then
    points := points + 1;
  end if;

  return least(points, 9);
end;
$$;
...
with scored as (
  select
    ...
    case
      when f.actual_home_goals is not null
       and f.actual_away_goals is not null
       and pr.pred_home_goals = f.actual_home_goals
       and pr.pred_away_goals = f.actual_away_goals
      then 1 else 0
    end as exact_hit,
    case
      when f.actual_home_goals is not null
       and f.actual_away_goals is not null
      then abs(pr.pred_home_goals - f.actual_home_goals) + abs(pr.pred_away_goals - f.actual_away_goals)
      else 999
    end as distance
  from participants p
...
select
  ...
  row_number() over (
    order by
      coalesce(sum(points), 0) desc,
      coalesce(sum(exact_hit), 0) desc,
      coalesce(sum(distance), 0) asc,
      full_name asc
  ) as rank_order
from scored
